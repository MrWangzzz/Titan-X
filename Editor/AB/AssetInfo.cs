...
namespace TitanX
{
    public class AssetInfo
    {
        ...
        public bool IsIndependentAsset { get; set; } = false;
        ...
        public void SetAssetBundleName(int pieceThreshold)
        {
            ...
            else
            {
                ...
                string abname = this.assetPath.Substring(0, this.assetPath.Length - extension.Length).Replace("/", ".") + ".ab";
                //是独立资源需要单独读取
                if (this.IsIndependentAsset)
                {
                    ai.SetAssetBundleNameAndVariant(abname, string.Empty);
                }
                //不是图集，而且大于阀值
                else if (this.parentSet.Count >= pieceThreshold)
                {
                    ai.SetAssetBundleNameAndVariant(abname, string.Empty);
                }
                //根节点
